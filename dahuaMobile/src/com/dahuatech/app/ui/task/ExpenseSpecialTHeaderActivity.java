...
import com.dahuatech.app.business.ExpenseSpecialTHeaderBusiness;
...
/**
 * @ClassName ExpenseSpecialTHeaderActivity
 * @Description 对私报销单据表头Activity
 * @author 21291
 * @date 2014年9月16日 上午10:43:10
 */
public class ExpenseSpecialTHeaderActivity extends MenuActivity implements ITaskContext{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.expensespecialtheader);
		...
		//获取服务地址
		serviceUrl=AppUrl.URL_API_HOST_ANDROID_EXPENSESPECIALTHEADERACTIVITY;	
		...
		//初始化帧布局实例 
		fCostTypeLinearLayout=(LinearLayout)findViewById(R.id.expensespecialtheader_LinearLayout);	
		if(!StringUtils.isEmpty(fMenuID) && !StringUtils.isEmpty(fSystemType) && !StringUtils.isEmpty(fBillID) && !StringUtils.isEmpty(fClassTypeId)&&!StringUtils.isEmpty(fStatus)){
			aTaskContext=AsyncTaskContext.getInstance(ExpenseSpecialTHeaderActivity.this, ExpenseSpecialTHeaderActivity.this);
			aTaskContext.callAsyncTask(serviceUrl);
		}
		else {
			UIHelper.ToastMessage(ExpenseSpecialTHeaderActivity.this, R.string.expensespecialtheader_netparseerror);
			return;
		}
	}
	...
	public Base getDataByPost(String serviceUrl) {
		...
		// 发送这个消息到消息队列中
		FactoryBusiness<?> factoryBusiness=FactoryBusiness.getFactoryBusiness(ExpenseSpecialTHeaderActivity.this);
		ExpenseSpecialTHeaderBusiness eBusiness= (ExpenseSpecialTHeaderBusiness)factoryBusiness.getInstance("ExpenseSpecialTHeaderBusiness",serviceUrl);
		return eBusiness.getExpenseSpecialTHeaderInfo(taskParam);
	}

	@Override
	public void initBase(Base base) {
		ExpenseSpecialTHeaderInfo extHeaderInfo=(ExpenseSpecialTHeaderInfo)base;
		if(!StringUtils.isEmpty(extHeaderInfo.getFBillNo())){
			fBillNo=(TextView)findViewById(R.id.expensespecialtheader_FBillNo);
			fBillNo.setText(extHeaderInfo.getFBillNo());
		}
		
		if(!StringUtils.isEmpty(extHeaderInfo.getFConSmName())){
			fConSmName=(TextView)findViewById(R.id.expensespecialtheader_FConSmName);
			fConSmName.setText(extHeaderInfo.getFConSmName());
		}
		
		if(!StringUtils.isEmpty(extHeaderInfo.getFCommitDate())){
			fCommitDate=(TextView)findViewById(R.id.expensespecialtheader_FCommitDate);
			fCommitDate.setText(extHeaderInfo.getFCommitDate());
		}
		
		if(!StringUtils.isEmpty(extHeaderInfo.getFGeneralName())){
			fGeneralName=(TextView)findViewById(R.id.expensespecialtheader_FGeneralName);
			fGeneralName.setText(extHeaderInfo.getFGeneralName());
		}
		
		if(!StringUtils.isEmpty(extHeaderInfo.getFAmountAll())){
			fAmountAll=(TextView)findViewById(R.id.expensespecialtheader_FAmountAll);
	...
	private void attachRelativeLayout(final ExpenseCostTypeInfo costTypeInfo){
		...
		imgButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(ExpenseSpecialTHeaderActivity.this, ExpenseSpecialTBodyActivity.class);
		...
		tv1.setTextAppearance(ExpenseSpecialTHeaderActivity.this, R.style.iconstext);
		...
		tv3.setTextAppearance(ExpenseSpecialTHeaderActivity.this, R.style.iconsamount);
		...
		appButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if("0".equals(fStatus)){ //显示待审批页面
					Intent intent = new Intent(ExpenseSpecialTHeaderActivity.this, WorkFlowActivity.class);
				...
				{//显示已审批页面
					Intent intent = new Intent(ExpenseSpecialTHeaderActivity.this, WorkFlowBeenActivity.class);
					intent.putExtra(AppContext.WORKFLOW_FSYSTEMTYPE_KEY, fSystemType);
					intent.putExtra(AppContext.WORKFLOW_FCLASSTYPEID_KEY, fClassTypeId);
					intent.putExtra(AppContext.WORKFLOW_FBILLID_KEY, fBillID);
	...
	*/
	...
	protected void onResume() {
		super.onResume();
		commonMenu.setContext(ExpenseSpecialTHeaderActivity.this);
	}
