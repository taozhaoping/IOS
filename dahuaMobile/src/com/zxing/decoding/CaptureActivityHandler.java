...
public final class CaptureActivityHandler extends Handler {
	...
	public void handleMessage(Message message) {
		switch (message.what) {
			case R.id.capture_restart_preview: // 准备进行下一次扫描			
				Log.d(TAG, "Got restart preview message");
				restartPreviewAndDecode();
				break;
			case R.id.capture_decode_succeeded:
				...
				break;
			case R.id.capture_decode_failed:
				// We're decoding as fast as possible, so when one decode fails,
				// start another.
				state = State.PREVIEW;
				cameraManager.requestPreviewFrame(decodeThread.getHandler(),R.id.capture_decode);
				break;
			case R.id.capture_return_scan_result:
				Log.d(TAG, "Got return scan result message");
				activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
				activity.finish();
				break;
			case R.id.capture_launch_product_query:
	...
	public void quitSynchronously() {
		...
		Message quit = Message.obtain(decodeThread.getHandler(), R.id.capture_quit);
		quit.sendToTarget();
